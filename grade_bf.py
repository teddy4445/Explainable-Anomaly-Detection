...
def vizualize_explanation(i_good, i_med, i_bad, print_good=True, print_med=True, print_bad=True):
    if print_good:
        print('\nIndex Good: ', i_good)
...
if __name__ == '__main__':
    ...
    # compute_scores(filename=filename, output_filename='grade_bf_3_new.csv')

    scores_filename = 'grade_bf_3.csv'
    scores_inf = pd.read_csv(scores_filename)
    d_inf = pd.read_csv(filename)

    i = 15
    vizualize_explanation(i_good=i,
                          i_med=len(scores_inf) // 2 - i,
                          i_bad=len(scores_inf) - 1 - i,
